...
{
 if (cdcgmcountercb)
  return cdcgmcountercb ( CD_CGM, (ftell ( cgm.fp )*100.)/cgm.file_size );
...
static void cgmb_do_nothing ( void )
{
 cgm.bc=cgm.len;
}
...
int cgmb_getbit ( unsigned char *b )
{
 ...
 if ( cgm.pc==0 || cgm.pc==8 )
  {
   b1 = cgm.buff.dados[cgm.bc];
   cgm.bc++;
 ...
 if ( cgm.len )
  {
   if ( cgm.len>cgm.buff.size )
     cgm.buff.dados = (char *) realloc ( cgm.buff.dados, sizeof(char) * cgm.len );

   erro = fread ( cgm.buff.dados, 1, cgm.len, cgm.fp );
   if ( erro<cgm.len ) return 1;

   cgm.bl += cgm.len;

   if ( len & 1 )
    {
     erro = fread ( &dummy, 1, 1, cgm.fp );
     if ( erro<1 ) return 1;
   ...
   while ( cont )
    {
     ...
     erro = fread ( &cgm.buff.dados[old_len], 1, len, cgm.fp );
 ...
 while ( cont )
  {
   ...
   s = (char *) my_realloc ( (unsigned char *)s, (sizeof ( char ) * l1) + 1 );
...
int cgmt_i ( long *i )
{
 ...
 if ( fscanf( cgm.fp, "%ld", i ) ) return 0;
...
int cgmt_r ( double *f )
{
 ...
 if ( fscanf( cgm.fp, "%lg", f ) ) return 0;
