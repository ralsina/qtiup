...
/****************************/
static int quick_map(byte *red, byte *green, byte *blue, int w, int h, byte *map, byte *rmap, byte *gmap, byte *bmap, int *maxcol)
{
...
static void   slow_fill_histogram PARM((byte*, byte*, byte*, int));
...
static void   slow_map_pixels PARM((byte*, byte*, byte*, int, int, byte*));
static void   init_error_limit PARM((void));


/* Master control for slow quantizer. */
static int slow_quant(byte *red, byte *green, byte *blue, int w, int h, byte *map, byte *rm, byte *gm, byte *bm, int *descols)
{
...
static void slow_fill_histogram (register byte *red, register byte *green, register byte *blue, int numpixels)
{
...
static void slow_map_pixels (byte *red, byte *green, byte *blue, int width, int height, byte *map)
{
...
#ifdef __CD__
  #include "cd.h"
  #define xxRGB2Map cdRGB2Map
  #define xxEncodeColor cdEncodeColor
#else
  #include "im.h"
  #define xxRGB2Map imRGB2Map
  #define xxEncodeColor imEncodeColor
#endif

void xxRGB2Map(int width, int height, unsigned char *red, unsigned char *green, unsigned char *blue, unsigned char *map, int pal_size, long *colors)
{
  int i, err;
  ...
  for (i=0; i < num_colors; i++)
    *colors++ = xxEncodeColor(rm[i], gm[i], bm[i]);
  ...
  /* this function should be changed to return the actual number of colors */
  /* return num_colors; */
}
