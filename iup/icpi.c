 ...
 * $Id: icpi.c,v 1.37 2006/01/07 20:10:10 scuri Exp $
 */
...
typedef Ihandle* (*itype_create)         (Iclass*, Ihandle** children);
typedef void     (*itype_destroy)        (Ihandle*);
typedef void     (*itype_map)            (Ihandle*, Ihandle* parent);
typedef void     (*itype_unmap)          (Ihandle*);
typedef void     (*itype_setattr)        (Ihandle*, char* attr, char* value);
typedef char*    (*itype_getattr)        (Ihandle*, char* attr);
typedef char*    (*itype_getdefaultattr) (Ihandle*, char* attr);
typedef int      (*itype_popup)          (Ihandle*, int x, int y);

/* Iclass definition */
struct Iclass_ {
   char* name;
   char* format; /* n: name, s: string, c: interface control */
...
Ihandle *iupCpiDefaultCreate (Iclass *ic, Ihandle** array)
{
  return IupCanvas("");
}
...
int iupCpiSetClassMethod(Iclass* iupCpiclass, char *method, Imethod func)
{
