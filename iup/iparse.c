 ...
 * $Id: iparse.c,v 1.36 2006/03/14 21:28:02 scuri Exp $
 */
...
static Ihandle* parsecontrol(Iclass *ic)
{
  ...
  { 
    ...
    arg[i]=NULL;
    result = IupCreatev(iupCpiGetClassName(ic), (Ihandle**)arg); /* ver isso aqui... o name nao esta la pra todos... arg e' um vetor de string? */
    if(arg != NULL)
      free(arg);
    return result;
  }
...
static Ihandle* iParseFunction (char* fntype, Iclass *ic)
{
 ...
 {
    unsigned int r,g,b;
    RETURN_IF_ERRO(iupLexMatch (TK_NAME)); r=(unsigned int)iupLexInt();
    RETURN_IF_ERRO(iupLexMatch (TK_COMMA));
    RETURN_IF_ERRO(iupLexMatch (TK_NAME)); g=(unsigned int)iupLexInt();
    RETURN_IF_ERRO(iupLexMatch (TK_COMMA));
    RETURN_IF_ERRO(iupLexMatch (TK_NAME)); b=(unsigned int)iupLexInt();
    n = IupColor (r,g,b);
 }
 else if(fntype == IMAGE_)
 {
    ...
    if (pixmap != NULL)
    {
      ...
      n = IupImage (width,height,(char*)pixmap);
      free(pixmap);
    }
 }
