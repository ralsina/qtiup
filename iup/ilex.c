 ...
 * $Id: ilex.c,v 1.32 2006/03/20 21:43:57 scuri Exp $
 */
...
int iupLexStart(char* filename)			/* initialize lexical analysis */
{
...
void iupLexRegisterLED (char *name, Iclass *ic)
{
  char lowercase[80];
  int i;
  for (i=0; *name; ++name, ++i)
...
static char* iLexFunction (char *f)
{
  ...
  else if (iupStrEqualNoCase (f, "color"))     return COLOR_;
