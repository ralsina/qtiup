 ...
 * $Id: immark.c,v 1.19 2006/02/03 21:51:29 uid20021 Exp $
 */
...
void iupmatMarkCellSet(Tmat *mat,int lin,int col, int mark)
{
  if (!mat->valcb)
    ...
    mat->markeditcb(mat->canvash,lin+1,col+1, mark);
  else
  {
    char str[100];
    sprintf(str, "MARK%d:%d", lin, col);
    if (mark)
      iupSetEnv(mat->canvash, str, "1");
    else
      iupSetEnv(mat->canvash, str, NULL);
  }
}
...
int iupmatMarkCellGet(Tmat *mat,int lin,int col)
{
  ...
  {
    if (mat->markcb)
      return mat->markcb(mat->canvash,lin+1,col+1);
    else
    {
      char str[100];
      sprintf(str, "MARK%d:%d", lin, col);
      return iupGetEnvInt(mat->canvash, str);
    }
  }
}
