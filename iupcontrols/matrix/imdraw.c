 ...
 * $Id: imdraw.c,v 1.53 2006/02/03 21:51:29 uid20021 Exp $
 */
...
typedef int (*Drawcb)(Ihandle *h, int lin, int col,int x1, int x2, int y1, int y2);
...
void iupmatSetCdFrameColor(Ihandle *h)
{
  ...
  cdForeground(cdEncodeColor((unsigned char) r, (unsigned char) g, (unsigned char) b));
}
...
static int CallDrawCB(Ihandle *h, Tmat *mat, int cor, int lin, int col, int x1, int x2, int y1, int y2)
{
  ...
  if (cb)
  {
    ...
    if (cb(h, lin, col, x1, x2, INVY(y1), INVY(y2)) == IUP_DEFAULT)
    {
      cdClip(CD_CLIPOFF);
      return 0;
    }
...
/*
%F Seta a cor de frente do CD para a cor correta para desenharmos esta
   celula de titulo. Esta funcao distingue entre celulas marcadas e nao
   marcadas por si so'.

%i h       - Handle da matriz
   lin,col - Coordenandas da celula.
...
static unsigned long SetTitleBgColor(Ihandle *h, int lin, int col)
{
  ...
  {
    /* Testa se ha uma cor especifica para esta celula.
      Se nao houver, pega a cor de fundo do dialogo.
    */
    sprintf(colorattr,"BGCOLOR%d:%d",lin,col);
    if (iupGetEnvRaw(h, colorattr))
      iupGetColor(h,colorattr,&r,&g,&b);
    else
      iupGetColor(IupGetDialog(h),"BGCOLOR",&r,&g,&b);
  }

  /* Se a celula estiver marcada, escurece a mesma em 20% */
  if((lin == 0 && iupmatMarkColumnMarked(h,col)) || (col == 0 && iupmatMarkLineMarked(h,lin)))
  {
  ...
  return cdForeground(cdEncodeColor((unsigned char)r,(unsigned char)g,(unsigned char)b));
}
...
static unsigned long SetTitleFgColor(Ihandle *h, int lin, int col)
{
  ...
  {
    sprintf(colorattr,"FGCOLOR%d:%d",lin,col);
    if (iupGetEnvRaw(h,colorattr))
    {
      iupGetColor(h,colorattr,&r,&g,&b);
      return cdForeground(cdEncodeColor((unsigned char)r,(unsigned char)g,(unsigned char)b));
...
%o Retorna a antiga cor de frente do CD.
*/
static unsigned long SetFgColor(Ihandle *h, int lin, int col, int type, int mark)
{
  char colorattr[30];
  unsigned int r,g,b;
  float factor;
  ...
  {
    if (type == COR_TITULO)
      return SetTitleFgColor(h,lin,col);

    /* 1 -  testa se ha uma cor especifica para esta celula */
    sprintf(colorattr,"FGCOLOR%d:%d",lin,col);
    if (iupGetEnvRaw(h,colorattr))
      iupGetColor(h,colorattr,&r,&g,&b);
    else
    {
      /* 2 - testa se ha uma cor definida para esta linha */
      sprintf(colorattr,"FGCOLOR%d:*",lin);
      if(iupGetEnvRaw(h,colorattr))
        iupGetColor(h,colorattr,&r,&g,&b);
      else
      {
        /* 3 - testa se ha uma cor definida para esta coluna */
        sprintf(colorattr,"FGCOLOR*:%d",col);
        if(iupGetEnvRaw(h,colorattr))
          iupGetColor(h,colorattr,&r,&g,&b);
        else
          iupGetColor(h,"FGCOLOR",&r,&g,&b);
      }
    }
  }

  if (mark)
    factor = GetAttenuation(h);
	else 
		factor = ATENUATION_NO;
  
  return cdForeground(cdEncodeColor((unsigned char)(factor*(double)r),
                                    (unsigned char)(factor*(double)g),
                                    (unsigned char)(factor*(double)b)));
}
...
static unsigned long SetBgColor(Ihandle *h, int lin, int col, int type, int mark)
{
  ...
  {
    if (type == COR_TITULO)
      return SetTitleBgColor(h,lin,col);
    ...
    {
      /* 2 - testa se ha uma cor definida para esta linha */
      sprintf(colorattr,"BGCOLOR%d:*",lin);
      if(iupGetEnvRaw(h,colorattr))
        iupGetColor(h,colorattr,&r,&g,&b);
      else
      {
        /* 3 - testa se ha uma cor definida para esta coluna */
        sprintf(colorattr,"BGCOLOR*:%d",col);
        if(iupGetEnvRaw(h,colorattr))
          iupGetColor(h,colorattr,&r,&g,&b);
        else
          iupGetColor(h,"BGCOLOR",&r,&g,&b);
      }
    }
  }
...
static unsigned long SetEmptyAreaColor(Ihandle *h)
{
  unsigned int r,g,b;
...
static void SetFont(Ihandle *h, int lin, int col)
{
  ...
  /* 1 -  testa se ha uma fonte especifica para esta celula */
  sprintf(attr,"FONT%d:%d",lin,col);
  v = iupGetEnvRaw(h,attr);
  if (v)
    cdNativeFont(v);
  else
  {
    /* 2 - testa se ha uma fonte definida para esta linha */
    sprintf(attr,"FONT%d:*",lin);
    v = iupGetEnvRaw(h,attr);
    if (v)
      cdNativeFont(v);
    else
    {
      /* 3 - testa se ha uma fonte definida para esta coluna */
      sprintf(attr,"FONT*:%d",col);
      v = iupGetEnvRaw(h,attr);
      if (v)
        cdNativeFont(v);
      else
        cdNativeFont(IupGetAttribute(h,"FONT"));
    }
  }
}
...
static void DrawText(Ihandle *h,int x1, int x2, int y1, int y2, char *text, int alignment, int xc, int cor, int lin, int col)
{
  ...
  if (text && *text)
  {
    ...
    if((lin > 0 && mat_lininactive(mat)[lin-1]) ||
      ...
      cdForeground(CD_INACTIVE_COLOR);
    else if (cor == COR_ELEM)
      SetFgColor(h,lin,col,cor,0);
    else if (cor == COR_REVERSE)
      oldbgc = SetFgColor(h,lin,col,cor,1);
    else
      SetFgColor(h,lin,col,cor,0);
...
static void DrawSort(Ihandle *h,int x2, int y1, int y2, int xc, int cor, int lin, int col, char* sort)
{
  Tmat *mat=(Tmat*)matrix_data(h); /* Usado pela macro CdBox */
  int yc;
   ...
   if(mat_ht(mat))
   {
...

     {
       ...
       if (sort && !iupStrEqual(sort, "NO"))
         DrawSort(h,x1+mat_w(mat)[j], y1, y2, x2, COR_TITULO, 0, j+1, sort);
     }
   }
   x1=x2;
 }
...
void iupmatDrawMatrix(Ihandle *h,int modo)
{
 ...
 if ((mat_nl(mat) == 0) || (mat_nc(mat) == 0))
 {
   char* bgcolor;
   unsigned int r,g,b;
   bgcolor = IupGetAttribute (IupGetParent(h), IUP_BGCOLOR);
   if (!bgcolor) bgcolor = IupGetAttribute(IupGetDialog(h), IUP_BGCOLOR);  /* the dialog has a default value */
   iupGetRGB(bgcolor, &r, &g, &b);
   cdBackground(cdEncodeColor((unsigned char) r, (unsigned char) g, (unsigned char) b));
   cdClear();
 }
...
void iupmatDrawFocus(Ihandle *h, int lin, int col, int coloca)
{
  ...
  if (coloca)          /* Coloca o desenho do foco */
    oldbgc = SetFgColor(h,lin+1,col+1,COR_ELEM,iupmatMarkCellGet(mat,lin,col));
...
char* iupmatDrawGetFgColor(Ihandle *h, int lin, int col)
{
  ...
  /* Na hora de buscar os atributos de cor, olha somente no environment,
     e nao no driver, para evitar uma recursao causada pelo metodo de
     getattribute.
  */
  do
  {
    ...
    val = IupGetAttribute(h,IUP_FGCOLOR);
  }while(0);
...
char* iupmatDrawGetBgColor(Ihandle *h, int lin, int col)
{
  ...
  */
  do
  {
    ...
    val = IupGetAttribute(h,IUP_BGCOLOR);
  }while(0);
...
char* iupmatDrawGetFont(Ihandle *h, int lin, int col)
{
  ...
  /* Na hora de buscar os atributos de fonte, olha somente no environment,
     e nao no driver, para evitar uma recursao causada pelo metodo de
     getattribute.
  */
  do
  {
    ...
    val = IupGetAttribute(h,IUP_FONT);
  }while(0);
