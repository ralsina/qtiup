 ...
 * $Id: imscroll.c,v 1.27 2006/01/19 22:03:21 scuri Exp $
 */
...
void iupmatScroll(Ihandle *h, int dir, int ref, int num)
{
  ...
  if (dir == SCROLL_LEFT)
  {
    ...
    cdScrollImage(x1,x2,y1,y2,mat_w(mat)[mat_fc(mat)],0);

    iupmatDrawColumnTitle(h,mat_fc(mat),mat_fc(mat));

    iupmatSetCdFrameColor(h);
    cdLine(XmaxCanvas(mat),y1,XmaxCanvas(mat),y2);
  ...
  else if (dir == SCROLL_UP)
  {
    ...
    cdScrollImage(x1,x2,y1,y2,0,-mat_h(mat)[mat_fl(mat)]);
  ...
  else if (dir == SCROLL_RIGHT)
  {
    ...
    cdScrollImage(x1,x2/*-1*/,y1,y2,mat_wt(mat)-x1,0);
  ...
  else if (dir == SCROLL_DOWN)
  {
    ...
    cdScrollImage(x1,x2/*-1*/,y1,y2,0,YmaxCanvas(mat)-mat_ht(mat)-y2);
...
int iupmatScrollCb(Ihandle *hm, int action, float x, float y)
{
  ...
  { cdFlush(); mat->redraw = 0; } /* always update */
