...
* $Id: iupcolorbar.c,v 1.8 2006/03/15 02:51:55 scuri Exp $
*/
...
static int colorbar_resize(Ihandle *h) {
  ...
  /* If needed, this part creates the CD canvas */
  if (obj->cdcanvas == NULL) { 
     obj->cdbufcanvas = cdCreateCanvas(CD_IUP, h);
     obj->cdcanvas = cdCreateCanvas(CD_DBUFFER, obj->cdbufcanvas);
     if (obj->cdcanvas == NULL) return IUP_IGNORE;
  }

  old_canvas = cdActiveCanvas();
  if (cdActivate(obj->cdcanvas) == CD_ERROR) return IUP_DEFAULT;
  cdGetCanvasSize(&obj->w, &obj->h, NULL, NULL);
...
static void call_repaint(Tcolorbar* obj, int preview, int idx)
{
  ...
  if (cdActivate(obj->cdcanvas) == CD_ERROR) return;
...
static Ihandle *colorbar_create(Iclass *ic, void **params) {
  ...
  for (i = 0; i < DEFAULT_NUM_CELLS; i++) {
    obj->colors[i] = cdEncodeColor(default_colors[i].r, default_colors[i].g, default_colors[i].b);
  }
...
int ymax) {
  ...
  cdIupDrawSunkenRect(xmin, ymin, xmax, ymax, obj->light_shadow, obj->mid_shadow, obj->dark_shadow);
}
...
int ymax, int idx) {
  ...
  if (col == obj->transparency) { 
     int xm = (xmin+xmax)/2;
     int ym = (ymin+ymax)/2;
     cdForeground(0xeeeeee);
     cdBox(xmin, xm, ymin, ym);
     cdBox(xm, xmax, ym, ymax);
     cdForeground(0xcccccc);
     cdBox(xmin, xm, ym, ymax);
     cdBox(xm, xmax, ymin, ym);
  }
  else {
    cdForeground(col);
    cdBegin(CD_FILL);
    cdVertex(xmin, ymin); cdVertex(xmin, ymax);
    cdVertex(xmax, ymax); cdVertex(xmax, ymin);
    cdEnd();
  }
