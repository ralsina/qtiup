 ...
 * $Id: iupmask.c,v 1.30 2006/03/15 02:51:55 scuri Exp $
 */
...
static int maskSet(Ihandle* h, char* mask, int autofill, int casei, int lin, int col)
{
  Icallback oldf = NULL;
  iMask* fsm;
  iupMask* iup_mask;
  ...
  iup_mask->is_matrix = iupStrEqual(IupGetType(h),"matrix");
  ...
  */
  ...
  else
  {
    ...
    if (iupStrEqual(IupGetType(h), "list"))
