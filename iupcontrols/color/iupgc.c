 ...
 * $Id: iupgc.c,v 1.19 2006/03/15 02:51:55 scuri Exp $
 */
...
{
  if (dither) {
    cdPutImageMap(FSW, FSH, con->c_map, sys_pal, 0, 0, FSW, FSH);
  } else {
    cdPutImageRGB(FSW, FSH, con->c_red, con->c_green, con->c_blue, 0, 0, FSW, FSH);
  }
}
...
static int c_map(Ihandle *s)
{
  ...
  con->c_cv = cdCreateCanvas(CD_IUP, con->c_icv);
  assert(con->c_cv != NULL);
...
static Ihandle *create_element(p_context con)
{
  ...
  IupSetCallback(con->c_icv, "MAP_CB", (Icallback)c_map);
