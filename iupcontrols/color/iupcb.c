 ...
 * $Id: iupcb.c,v 1.23 2006/03/15 02:51:55 scuri Exp $
 */
...
/* workaround do bug do CD que voa com cdActivate(NULL) */
#define cdActivate(x) if (x != NULL) cdActivate(x)
...
static int  hls_button(Ihandle *s, int b, int e, int x, int y, char *r);
static int  hls_motion(Ihandle *s, int x, int y, char *r);
static int  hls_repaint(Ihandle *s) ;
static int  hls_keypress(Ihandle *s, int c, int press);
static int  hls_map(Ihandle *s);

static void h_draw(p_context con);
static void h_mark(p_context con, int x, int y);
static void h_unmark(p_context con);
static int  h_button(p_context con, int e, int x, int y, char *r);
static int  h_motion(p_context con, int x, int y, char *r);
...
static int  ls_button(p_context con, int e, int x, int y, char *r);
static int  ls_motion(p_context con, int x, int y, char *r);
...
static void h_mark(p_context con, int x, int y)
{
  cdSector(x, y, MS, MS, 0, 360);
  con->h_x = x; con->h_y = y;
}

static void h_unmark(p_context con)
{
  cdSector(con->h_x, con->h_y, MS, MS, 0, 360);
}
...
static void ls_unmark(p_context con)
{
  cdSector(con->ls_x, con->ls_y, MS, MS, 0, 360);
}
...
static void h_draw(p_context con)
{
  ...
  if (dither) {
    cdPutImageMap(H, H, h_map, sys_pal, 0, 0, H, H);
  } else {
    cdPutImageRGB(H, H, h_red, h_green, h_blue, 0, 0, H, H);
  }
...
static void ls_draw(p_context con)
{
  ...
  cdWriteMode(CD_REPLACE);
  if (dither) {
    cdPutImageMap(LS, LS, con->ls_map, sys_pal, SD/5+CL+DT, SD/10+CL+DT, LS, LS);
  } else {
    cdPutImageRGB(LS, LS, con->ls_red, con->ls_green, con->ls_blue, SD/5+CL+DT, SD/10+CL+DT, LS, LS);
  }
  cdWriteMode(CD_NOT_XOR);
  h_unmark(con);
  ls_unmark(con);
...
static int hls_button(Ihandle *s, int b, int e, int x, int y, char *r)
{
  ...
  if (!con->h_down && e && in_h(x, y)) {
    cdWriteMode(CD_NOT_XOR);
    h_button(con, 1, x, y, r);
    con->h_down = 1;
  } 
  if (con->h_down && !e) {
    h_button(con, 0, x, y, r);
    con->h_down = 0;
    cdActivate(con->old_cv);
  }
  if (!con->ls_down && e && in_ls(x, y)) {
    cdWriteMode(CD_NOT_XOR);
    ls_button(con, 1, x, y, r);
    con->ls_down = 1;
  } 
  if (con->ls_down && !e) {
    ls_button(con, 0, x, y, r);
    con->ls_down = 0;
  }
...
static int hls_repaint(Ihandle *s) 
{
  ...
  if (cdGetColorPlanes() < 16) {
    cdPalette(pal_size, sys_pal, CD_FORCE);
    if (!dither) {
...
static void init_usr_pal(void)
{
  ...
  shade_dr = 2*(r/3); shade_dg = 2*(g/3); shade_db = 2*(b/3);
  shade_lr = (255+r)/2; shade_lg = (255+g)/2; shade_lb = (255+b)/2;
...
{
  p_context con = get_context(s);
  con->hls_cv = cdCreateCanvas(CD_IUP, con->hls_icv);
...
static Ihandle *create_element(p_context con)
{
  ...
  IupSetCallback(con->hls_icv, "MAP_CB", (Icallback) hls_map);
