...
typedef int (*TDrawCb)(Ihandle*, int, int, int, int, int, int);
...
static int cells_scroll(Ihandle *h, int op, float posx, float posy);
static int cells_repaint(Ihandle *h);
static int cells_resize(Ihandle *h, int sx, int sy);
...
static char* cells_getattrib(Ihandle *h, char *attr) {
  ...
  if (iupStrEqual(attr, IUP_BOXED)) { 
     ...
     return (char*) obj->img_canvas;
  }
  else if (iupStrEqual(attr, IUP_CANVAS)) { 
     return (char*) obj->cd_canvas;
  }
...
static void cells_setattrib(Ihandle *h, char *attr, char *value) {
  ...
  if (iupStrEqual(attr, IUP_BUFFERIZE)) { 
     if (value == NULL || iupStrEqual(value, IUP_NO)) { 
        obj->bufferize = 0;
        adjust_scrolls(obj);
        repaint_all(obj);
     }
  ...
  else if (iupStrEqual(attr, IUP_DOUBLE_BUFFER)) { 
     if (value != NULL && iupStrEqual(value, IUP_NO)) obj->dbuffer = 0;
     else obj->dbuffer = 1;
  }
     ...
     if (value != NULL && iupStrEqual(value, IUP_NO)) obj->boxed = 0;
     else obj->boxed = 1;
     repaint_all(obj);
  }
  else if (iupStrEqual(attr, IUP_CLIPPED)) { 
     if (value != NULL && iupStrEqual(value, IUP_NO)) obj->clipped = 0;
     else obj->clipped = 1;
     repaint_all(obj);
  }
  else if (iupStrEqual(attr, IUP_FULL_VISIBLE)) { 
     int i, j;
     int read = sscanf(value, "%d:%d", &i, &j);
     if (read != 2) return;
...
static int cells_resize(Ihandle *h, int sx, int sy) {
  ...
  /* if cd canvas has not been created yet, it is now constructed */
  if (obj->cd_canvas == NULL) 
     obj->cd_canvas = cdCreateCanvas(CD_IUP, h);
  if (obj->cd_canvas == NULL) return IUP_IGNORE;

  /* if image canvas has not been created yet, it is now constructed */
  if (obj->img_canvas == NULL)
     obj->img_canvas = cdCreateCanvas(CD_DBUFFER, obj->cd_canvas);
  if (obj->img_canvas == NULL) return IUP_IGNORE;

  /* Updating cell internal values 
   * (the size is stored to avoid calling cd functions */
  obj->width  = sx;
  obj->height = sy;
...
static void cells_destroy(Ihandle *h) {
  ...
  if (obj->cd_canvas != NULL) 
  { 
    cdKillCanvas(obj->cd_canvas);
    obj->cd_canvas = NULL;
  }

  if (obj->img_canvas != NULL) 
  { 
    cdKillCanvas(obj->img_canvas);
    obj->img_canvas = NULL;
  }
...
static int cells_button(Ihandle *h, int b, int m, int x, int y, char* r) {
  ...
  /* Activating the cd canvas just to get mouse coordinates */
  old_canvas = cdActiveCanvas();
  if (cdActivate(obj->cd_canvas) == CD_ERROR) return IUP_DEFAULT;
  cdCanvas2Raster(&x, &y);  
  if (old_canvas) cdActivate(old_canvas);
...
int ymin, int ymax, int i, int j) {
  ...
  int oldclip = cdClip(CD_QUERY);
  cdGetClipArea(&oldxmin, &oldxmax, &oldymin, &oldymax);
  ...
  { 
     ...
     if (func)
       func(obj->self, i, j, xmin, xmax, ymin, ymax);
  }

  /* Restoring clipping attributes */
  cdClip(oldclip);
  cdClipArea(oldxmin, oldxmax, oldymin, oldymax);
}
 ...
 */
static void repaint_all(TCells* obj) {
  /* Saving current cd canvas and preparing new canvas to be activated */
  cdCanvas* old_canvas = cdActiveCanvas();
  cdCanvas* canvas = obj->dbuffer == 1 ? obj->img_canvas : obj->cd_canvas;
  ...
  if (cdActivate(canvas) == CD_ERROR) return;
  if (obj->self == NULL) return;

  if (obj->no_color == -1)
    obj->no_color = cdIupConvertColor(cdIupGetBgColor(obj->self));

  /* Getting object background color and repainting it.*/
  cdBackground(obj->no_color); 
  cdClear();
