 ...
 * $Id: itedit.c,v 1.4 2006/03/15 02:51:55 scuri Exp $
 */
...
static int treeTextActionCb(Ihandle* self, int c, char *after)
{
  ...
  {
    int ret;
    iupSetEnv(tree_canvash(tree), "_IUPTREE_CALL_RENAME", "1");
    ret = treecallRenameCb(tree_canvash(tree), after);
    iupSetEnv(tree_canvash(tree), "_IUPTREE_CALL_RENAME", NULL);
...
void treeEditCheckHidden(Ihandle* self)
{
  ...
  if (iupCheck(tree_texth(tree), "VISIBLE")==YES)
  {
    ...
    if (IupGetInt(tree_canvash(tree), "_IUPTREE_CALL_RENAME")==1)
      return;

    value = iupStrDup(IupGetAttribute(tree_texth(tree), IUP_VALUE));
    treecallRenameCb(tree_canvash(tree), value);
    free(value);
    IupSetAttribute(tree_texth(tree),IUP_VISIBLE,IUP_NO);
    IupSetAttribute(tree_texth(tree),IUP_ACTIVE,IUP_NO);
  }
