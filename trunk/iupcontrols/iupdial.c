...
* $Id: iupdial.c,v 1.32 2006/03/15 02:51:55 scuri Exp $
*/
...
{
  d->bgcolor = cdIupConvertColor(cdIupGetBgColor(self));
  ...
  cdActivate(d->cdim);
  cdBackground(d->bgcolor);
  cdClear();
}

static void putimage(Tdial *d, Ihandle* self)
{
  cdActivate(d->cdcanvas);
  cdPutImage(d->im,0,0);

  if (d->has_focus)
    cdIupDrawFocusRect(self, 0, 0, d->w-1, d->h-1);
}
...
static void drawdial(Tdial *d, Ihandle* self)
{
  ...
  if(d->cdcanvas == NULL || d->cdim == NULL)
    resize_cb(self);
...
static void drawvdialback(Tdial *d, double amin, double amax, int *ymin, int *ymax)
{
  ...
  *ymin = *ymax = d->h/2;
  for (a=amin;a<0.5*pi;a+=delta)		/* shading */
  {
    ...
    cdForeground(getcolor(d,a,amin));
    cdBox(SPACE+1, d->w-1-SPACE-2,y0,y1);
...
static void drawVdial(Tdial *d)
{
  ...
  cdIupDrawRaisenRect(SPACE, ymin, d->w-1-SPACE, ymax,
  ...
  for (;a<amax;a+=delta)		/* graduation */
  {
    ...
    cdIupDrawHorizSunkenMark(SPACE+1, d->w-1-SPACE-2, y, d->light_shadow, d->dark_shadow);
  }
}
...
static void drawhdialback(Tdial *d,double amin,double amax, int *xmin, int *xmax)
{
  ...
  *xmin = *xmax = d->w/2;
  for (a=amin;a<0.5*pi;a+=delta)
  {
    ...
    cdForeground(getcolor(d,a,amin));
    cdBox(x0,x1,SPACE+2,d->h-1-SPACE-1);
...
static void drawHdial(Tdial *d)
{
  ...
  cdIupDrawRaisenRect(xmin, SPACE, xmax, d->h-1-SPACE,
                      d->light_shadow, d->mid_shadow, d->dark_shadow);
  ...
  for (;a<amax;a+=delta)
  {
    ...
    cdIupDrawVertSunkenMark(x, SPACE+2, d->h-1-SPACE-1, d->light_shadow, d->dark_shadow);
  }
}

static void drawbol(Tdial *d, int x1, int y1)
{
  cdForeground(d->bgcolor);
  cdBox(x1, x1+4, y1, y1+4);

  cdForeground(d->light_shadow);
  cdLine(x1,y1+1,x1,y1+3);
  cdLine(x1+1,y1+4,x1+3,y1+4);

  cdForeground(d->mid_shadow);
  cdLine(x1+1,y1,x1+4,y1);
  cdLine(x1+4,y1,x1+4,y1+3);

  cdForeground(d->dark_shadow);
  cdLine(x1+2,y1,x1+3,y1);
  cdLine(x1+4,y1+1,x1+4,y1+2);
}
...
static void drawCdial(Tdial *d)
{
  ...
  cdForeground(d->mid_shadow);
  cdLineWidth(2);
  cdArc(xc,yc,wide-1,wide-1,-135,45.0);
  cdLineWidth(1);
  cdForeground(d->bgcolor);
  cdSector(xc,yc,wide-2,wide-2,0.0,360.0);
  cdForeground(d->light_shadow);
  cdArc(xc,yc,wide,wide,45,225);
  cdForeground(d->dark_shadow);
  cdArc(xc,yc,wide,wide,-135,45);
...
static int btp(Ihandle *self,int v,int x,int y,char *r)
{
  Tdial *d=(Tdial *)iupGetEnv(self,"_IUPDIAL_DATA");
  Aplcb aplbtp;
...
static int btr(Ihandle *self,int v,int x,int y,char *r)
{
  Tdial *d=(Tdial *)iupGetEnv(self,"_IUPDIAL_DATA");
  Aplcb aplbtr;
...
static int button_cb(Ihandle *self,int b,int m,int x,int y,char *r)
{
  if (m) return btp(self,b,x,y,r);
  else   return btr(self,b,x,y,r);
}
...
static int resize_cb(Ihandle *self)
{
  ...
  cdActivate(d->cdcanvas);
  cdGetCanvasSize(&d->w,&d->h,NULL,NULL);
  ...
  d->im=cdCreateImage(d->w,d->h);
  d->cdim=cdCreateCanvas(CD_IMAGE,d->im);
...
static void dialdestroy(Ihandle *h)
{
  ...
  if (d==NULL || d->cdcanvas==NULL) return;
  if (d->im!=NULL) cdKillImage(d->im);
  if (d->cdim!=NULL) cdKillCanvas(d->cdim);
  if (d->cdcanvas!=NULL) cdKillCanvas(d->cdcanvas);
...
static void dialsetattr(Ihandle *h, char *attr, char *value)
{
  ...
  if(IupGetAttribute(h, IUP_WID) != NULL && d->cdcanvas != NULL && d->cdim != NULL)
    drawdial(d, h);
}
