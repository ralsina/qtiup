 ...
 * $Id: iuptabs.c,v 1.98 2006/03/15 02:51:55 scuri Exp $
 */
...
#ifdef WIN32                            /* OLD STUFF */
#define FONT_ATTR "WIN_FONT"
#define FONT_ACTIVE_ATTR "WIN_FONT_ACTIVE"
#define FONT_INACTIVE_ATTR "WIN_FONT_INACTIVE"
#else
#define FONT_ATTR "MOTIF_FONT"
#define FONT_ACTIVE_ATTR "MOTIF_FONT_ACTIVE"
#define FONT_INACTIVE_ATTR "MOTIF_FONT_INACTIVE"
#endif
...
static const int TABS_TEXT_HEIGHT = 8;
...
{
  if (tab_font) 
    cdNativeFont(cdIupSetFontBold(tab_font, bold));
  else 
  {
    char *font = IupGetAttribute(tabs->self, IUP_FONT);
    if(font != NULL)
      cdNativeFont(cdIupSetFontBold(font, bold)); 
    else
      cdFont(CD_HELVETICA, bold? CD_BOLD: CD_PLAIN, TABS_TEXT_HEIGHT); 
  }
}
                              ...
                              tTabsType type, tTabs * tabs)
{
  ...
  {
  case TABS_TOP:
    cdForeground (tabs->mid_shadow);
    cdLine ((long)(x1 + step_x * 3.), y1, (long)(x1 + 4. * step_x), y2);
    cdLine ((long)(x1 + step_x), y1, (long)(x1 + 2. * step_x), y2);

    cdForeground (tabs->dark_shadow);
    cdLine (x1, y1, (long)(x1 + step_x), y2);
    cdLine ((long)(x1 + step_x), y1 - 1, (long)(x1 + 2. * step_x), y2 - 1);
    cdLine ((long)(x1 + step_x * 2.), y1, (long)(x1 + 3. * step_x), y2);
    cdLine ((long)(x1 + step_x * 3.), y1 + 1, (long)(x1 + 4. * step_x), y2 + 1);
    cdLine ((long)(x1 + step_x * 4.), y1, x2, y2);
    break;
  ...
  case TABS_LEFT:
    cdForeground (tabs->mid_shadow);
    cdLine (x1, (long)(y1 + 3. * step_y), x2, (long)(y1 + step_y * 4.));
    cdLine (x1, (long)(y1 + step_y), x2, (long)(y1 + step_y * 2.));

    cdForeground (tabs->dark_shadow);
    cdLine (x1, y1, x2, (long)(y1 + step_y));
    cdLine (x1 - 1, (long)(y1 + step_y), x2 - 1, (long)(y1 + step_y * 2.));
    cdLine (x1, (long)(y1 + step_y * 2.), x2, (long)(y1 + step_y * 3.));
    cdLine (x1 + 1, (long)(y1 + 3. * step_y), x2 + 1, (long)(y1 + step_y * 4.));
    cdLine (x1, (long)(y1 + 4. * step_y), x2, y2);
    break;
                        ...
                        tTabsType type)
{
  ...
  {
  case TABS_TOP:
    cdForeground (tabs->light_shadow);
    cdLine (x1, y1, x2, y2);
    break;
  case TABS_LEFT:
    cdForeground (tabs->light_shadow);
  ...
  case TABS_BOTTOM:
    ...
    cdForeground (tabs->mid_shadow);
    if (x1>x2)
      cdLine (x2+1, y1+1, x1-1, y2+1);
    else
      cdLine (x1+1, y1+1, x2-1, y2+1);
    break;
  case TABS_RIGHT:
    cdForeground (tabs->dark_shadow);
    cdLine (x1, y1, x2, y2);
...
static void DrawTab(tTabs * tabs, int tab_index, int offset)
{
  ...
  */
  ...
  FillArea (x1, y1, x2, y2, x3, y3, x4, y4);
  ...
  case BROKEN_END:
    ...
    {
    case TABS_TOP:
      cdClipArea (x1, x3 - TABS_BORDER - 1, y1, y3);
      break;
    case TABS_BOTTOM:
      cdClipArea (x2, x4 - TABS_BORDER - 1, y2, y4);
      break;
    case TABS_LEFT:
      cdClipArea (x3, x1, y3 + TABS_BORDER + 1, y1);
      break;
    case TABS_RIGHT:
      cdClipArea (x4, x2, y4 + TABS_BORDER + 1, y2);
      break;
    }
  ...
  case BROKEN_START:
    ...
    {
    case TABS_TOP:
      cdClipArea (x1 + TABS_BORDER + 1, x3, y1, y3);
      break;
    case TABS_BOTTOM:
      cdClipArea (x2 + TABS_BORDER + 1, x4, y2, y4);
      break;
    case TABS_LEFT:
      cdClipArea (x3, x1, y3, y1 - TABS_BORDER - 1);
      break;
    case TABS_RIGHT:
      cdClipArea (x4, x2, y4, y2 - TABS_BORDER - 1);
      break;
    }
  ...
  case BROKEN_CENTER:
    ...
    {
    case TABS_TOP:
      cdClipArea (x1 + TABS_BORDER + 1, x3 - TABS_BORDER - 1,
                  y1 + TABS_BORDER + 1, y3 - TABS_BORDER - 1);
      break;
    case TABS_BOTTOM:
      cdClipArea (x2 + TABS_BORDER + 1, x4 - TABS_BORDER - 1,
                  y2 + TABS_BORDER + 1, y4 - TABS_BORDER - 1);
      break;
    case TABS_LEFT:
      cdClipArea (x3 + TABS_BORDER + 1, x1 - TABS_BORDER - 1,
                  y3 + TABS_BORDER + 1, y1 - TABS_BORDER - 1);
      break;
    case TABS_RIGHT:
      cdClipArea (x4 + TABS_BORDER + 1, x2 - TABS_BORDER - 1,
  ...
  cdForeground (tabs->fgcolor); 

  if (tabs->tabs_orientation == TABS_VERTICAL)
    cdTextOrientation(90);
  else
    cdTextOrientation(0);

  {
    char* tab_font = tabs->font;
    int bold = 0;

    if(iupStrEqual(IupGetAttribute(tabs->tabs_line.tabs_info[tab_index].ihandle,IUP_ACTIVE),IUP_YES))
    {
      if (tab_index == tabs->current_tab)
      {
...
{
  long old_color = cdForeground (tabs->bgcolor);

  cdBegin (CD_FILL);
  cdVertex (x, y);
  cdVertex (x + width - 1, y);
  cdVertex (x + width - 1, y + height - 1);
  cdVertex (x, y + height - 1);
  cdEnd ();
...
static void DrawArrow (int x, int y, int width, int height, int type)
{
  long old_color = cdForeground (CD_BLACK);
  ...
  {
  case TABS_LEFT:
    cdVertex (x + 4, y + height / 2);
    cdVertex (x + width - 1 - 4, y + height - 1 - 4);
    cdVertex (x + width - 1 - 4, y + 4);
    break;

  case TABS_RIGHT:
    cdVertex (x + width - 1 - 4, y + height / 2);
    cdVertex (x + 4, y + height - 1 - 4);
    cdVertex (x + 4, y + 4);
    break;

  case TABS_TOP:
    cdVertex (x + 4, y + 4);
    cdVertex (x + width / 2, y + height - 1 - 4);
    cdVertex (x + width - 1 - 4, y + 4);
    break;

  case TABS_BOTTOM:
    cdVertex (x + 4, y + height - 1 - 4);
    cdVertex (x + width / 2, y + 4);
    cdVertex (x + width - 1 - 4, y + height - 1 - 4);
    break;
  }
...
static void DrawDots (int x, int y, int width, int height)
{
  x -= 1;
  y += height / 3 - 1;
  cdRect (x + 3, x+4, y+3, y + 4);
  cdRect (x + width / 2, x+ width/2 +1 , y+3, y + 4);
  cdRect (x + width - 1 - 3, x + width -1 - 2, y +3, y + 4);
}
...
static void DrawScrollbar (tTabs * tabs)
{
  ...
  if (tabs->tabs_orientation == TABS_HORIZONTAL)
  {
    ...
    DrawArrow(       x, y, x3, h, scroll_updown? TABS_TOP: TABS_LEFT);

    DrawButton(tabs, x + x3, y, x3, h );
    DrawDots(        x + x3, y, x3, h);

    DrawButton(tabs, x + 2 * x3, y, x3, h );
    DrawArrow(       x + 2 * x3, y, x3, h, scroll_updown? TABS_BOTTOM: TABS_RIGHT);
  }
  else
  {
    int y3 = h / 3;
    ...
    DrawArrow(       x, y, w, y3, scroll_updown? TABS_BOTTOM: TABS_RIGHT);

    DrawButton(tabs, x, y + y3, w,  y3 );
    DrawDots(        x, y + y3, w,  y3);

    DrawButton(tabs, x, y + 2 * y3, w,  y3 );
    DrawArrow(       x, y + 2 * y3, w,  y3, scroll_updown? TABS_TOP: TABS_LEFT);
  }
...
/*
*  Inicia processo de desenho
*/

static int StartDraw (tTabs * tabs)
{
  if (tabs->canvas == NULL)
    return 0;

  tabs->old_canvas = cdActiveCanvas ();

  if (tabs->image_canvas != NULL)
    cdActivate (tabs->image_canvas);
  else
    cdActivate (tabs->canvas);
...
static void ClearBackground (tTabs * tabs)
{
  ...
  cdBackground (cdIupConvertColor(cdIupGetParentBgColor(tabs->self)));
  cdClear ();
...
/*
*  Finaliza processo de desenho
*/

static void EndDraw (tTabs * tabs)
{
  if (tabs->image_canvas)
  {
    cdActivate (tabs->canvas);
    cdPutImage (tabs->image, 0, 0);
  }

  cdActivate (tabs->old_canvas);
  tabs->old_canvas = NULL;
}
...
static void init_tabs (tTabs * tabs)
{
  /* verifica se canvas foi mapeado */
  if (IupGetAttribute (tabs->self, IUP_WID) == NULL)
    return;

  /* cria canvas */
  tabs->canvas = cdCreateCanvas (CD_IUP, tabs->self);
...
static void calc_text_size(tTabs *tabs)
{
  ...
  iupdrvGetCharSize(tabs->tabs_line.tabs_info[0].ihandle, &text_width, &text_height);
  ...
  for(t = 0; t < tabs->number_of_tabs; t++)
  {
    ...
    cdTextSize(title, &text_width, NULL);
...
static void tabsUpdateImage(tTabs *tabs)
{
  if (tabs->image_canvas != NULL)
  {
    cdKillCanvas (tabs->image_canvas);
    cdKillImage (tabs->image);
  }

  tabs->old_canvas = cdActiveCanvas();
  cdActivate (tabs->canvas);
  tabs->image = cdCreateImage (tabs->w, tabs->h);
  tabs->image_canvas = cdCreateCanvas (CD_IMAGE, tabs->image);
...
static void tabssetcurrentsize (Ihandle * self, int max_w, int max_h)
{
  ...
  /* recria imagem utilizada em desenho offscreen */

  if (tabs->canvas != NULL && tabs->use_image)
    tabsUpdateImage(tabs);
...
static int repaint_cb (Ihandle * self)
{
  ...
  if (!iupStrEqual(IupGetType(self), "tabs"))
    return IUP_IGNORE;
...
/* this is also called for the resize callback */
...
static int keypress_cb (Ihandle * self, int c, int press)
{
  ...
  {
    int last = (tabs->current_tab < (tabs->number_of_tabs - 1));
    int result = cb_tabchange (tabs,
                               GetTabIhandle(tabs, last ? tabs->current_tab + 1 : 0 ),
                               GetTabIhandle(tabs, tabs->current_tab));
  ...
  {
    int first = tabs->current_tab == 0;
    int result = cb_tabchange (tabs,
                               GetTabIhandle(tabs, first? tabs->number_of_tabs - 1: tabs->current_tab - 1),
                               GetTabIhandle(tabs, tabs->current_tab));
    ...
    if (result != IUP_IGNORE)
    {
      if(tabs->current_tab >0)
        ...
        tabs->current_tab=tabs->number_of_tabs - 1;
      SetNewCurrentTab (tabs);
    }
  }
  ...
  */
      ...
      if (scroll_pos > 2 * scroll_space / 3)        /* botao de avanco */
      {
          ...
          /* caso contrario, muda o tab */
        {
          result = cb_tabchange (tabs,
            GetTabIhandle (tabs, last ? 0 : tabs->current_tab + 1 ),
            GetTabIhandle (tabs, tabs->current_tab));
  ...
  if (tabs->tabs_line.broken_start_visible == TABS_TRUE)
  {
        ...
        *virtual_my <= (tabs->tabs_line.line_thick - TABS_CURRENT_EXTRA_PIXELS - 1))
    {
      ...
      {
        int result = cb_tabchange (tabs, GetTabIhandle (tabs, tabs->tabs_line.start_tab - 1), 
                                        GetTabIhandle (tabs, tabs->current_tab));
        ...
        *virtual_mx <= (offset + tabs->tabs_line.tabs_info[count].tab_len - 1))
    {
      if (count != tabs->current_tab)
      {
          ...
          (tabs->tabs_line.line_thick - TABS_CURRENT_EXTRA_PIXELS - 1))
        {
          ...
          {
            int result = cb_tabchange (tabs, GetTabIhandle (tabs, count),
                                            GetTabIhandle (tabs, tabs->current_tab));
  ...
  if (tabs->tabs_line.broken_end_visible == TABS_TRUE)
  {
        ...
        *virtual_my <= (tabs->tabs_line.line_thick - TABS_CURRENT_EXTRA_PIXELS - 1))
    {
      ...
      {
        int result = cb_tabchange(tabs, GetTabIhandle (tabs, tabs->tabs_line.end_tab + 1),
                                        GetTabIhandle (tabs, tabs->current_tab));
...
static Ihandle * tabscreate(Iclass *tabs_class, void **params)
{
  Ihandle *self = NULL;
  tTabs *tabs = NULL;
  ...
  IupSetCallback(self, IUP_RESIZE_CB, (Icallback) map_cb);
...
static void tabsmap (Ihandle * self, Ihandle * parent)
{
  ...
  if (tabs->bgcolor == -1) 
  {
    tabs->bgcolor = cdIupConvertColor(cdIupGetBgColor(self));
...
static void tabssetattr (Ihandle * self, char *attr, char *value)
{
  ...
  else if (iupStrEqual (attr, "DOUBLEBUFFER"))
  {
  ...
  else if (iupStrEqual(attr, ICTL_FONT) || iupStrEqual(attr, FONT_ATTR))
  {
    ...
    if(native)
    {
      if(tabs->font)
        free(tabs->font);
      tabs->font = iupStrDup(native);
    }
  }
  else if (iupStrEqual(attr, ICTL_FONT_ACTIVE) || iupStrEqual(attr, FONT_ACTIVE_ATTR))
  {
    char *native = IupMapFont(value);
    if (!native) native = value;
    if(native)
  ...
  else if (iupStrEqual(attr, ICTL_FONT_INACTIVE) || iupStrEqual(attr, FONT_INACTIVE_ATTR))
  {
    char *native = IupMapFont(value);
    if (!native) native = value;
    if(native)
  ...
  if (tabs->canvas != NULL && do_repaint == TABS_TRUE)
    repaint_cb (self);
}
