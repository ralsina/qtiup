 ...
 * $Id: iupval.c,v 1.54 2006/03/24 01:39:47 scuri Exp $
 */
...
{
  v->bgcolor = cdIupConvertColor(cdIupGetBgColor(self));
  cdIupCalcShadows(v->bgcolor, &v->light_shadow, &v->mid_shadow, &v->dark_shadow);

  if (!v->active && !v->himage) 
  ...
  cdActivate(v->cdim);
  cdBackground(v->bgcolor);
  cdClear();
}

static void putimage(Tval *v, Ihandle* self)
{
  cdActivate(v->cdcanvas);
  cdPutImage(v->im,0,0);

  if (v->has_focus)
    cdIupDrawFocusRect(self, 0, 0, v->w-1, v->h-1);
}
...
{
  cdCanvas *old_canvas = cdActiveCanvas();

  if(v->cdcanvas == NULL || v->cdim == NULL)
    resize_cb(self);
  ...
  if(old_canvas != NULL)
    cdActivate(old_canvas);
}

static void vdrawtick(Tval *v, int y)
{
  cdIupDrawHorizSunkenMark(1, 1+TICK, y, v->light_shadow, v->dark_shadow);
  cdIupDrawHorizSunkenMark(v->w-2-TICK, v->w-2, y, v->light_shadow, v->dark_shadow);
}

static void hdrawtick(Tval *v, int x)
{
  cdIupDrawVertSunkenMark(x, 1, 1+TICK, v->light_shadow, v->dark_shadow);
  cdIupDrawVertSunkenMark(x, v->h-2-TICK, v->h-2, v->light_shadow, v->dark_shadow);
}
...
static void vdrawval(Tval *v)
{
  int y, x = (v->w-TRAIL)/2;
  double vn = (v->val-v->vmin)/(v->vmax-v->vmin);

  /* trail */
  cdIupDrawSunkenRect(x, TRAIL_START, x+TRAIL-1, TRAIL_END(v->h)+1, 
  ...
  if (v->active && v->himage) 
  {
    ...
    cdPutImageRectMap(v->himage_w, v->himage_h, v->himage, v->himage_colors,
                      x, y-v->himage_h/2, 0,0,0,0,0,0);
  } 
  else if (!v->active && v->himageinactive) 
  {
    x = (v->w - v->himageinactive_w)/2;
    y = (int)(vn*(TRAIL_LONG(v->h)-2) + TRAIL_START+1);
    cdPutImageRectMap(v->himageinactive_w, v->himageinactive_h, v->himageinactive, 
  ...
  else
  {
    ...
    y = (int)(vn*(TRAIL_LONG(v->h)-2) + TRAIL_START+1);
    cdIupDrawRaisenRect(x, y-HANDLER_LONG/2-1, x+HANDLER_SHORT-1, y+HANDLER_LONG/2, 
                        v->light_shadow, v->mid_shadow, v->dark_shadow);

    /* handler background */
    cdForeground( v->bgcolor );
    cdBox(x+1, x+HANDLER_SHORT-1-2, y-HANDLER_LONG/2+1, y+HANDLER_LONG/2-1);

    /* handler sunken mark */
    cdIupDrawHorizSunkenMark(x+2, x+HANDLER_SHORT-4, y, v->light_shadow, v->dark_shadow);
  }
}

static void hdrawval(Tval *v)
{
  int x, y = (v->h-TRAIL)/2;
  double vn = (v->val-v->vmin)/(v->vmax-v->vmin);

  /* trail */
  cdIupDrawSunkenRect(TRAIL_START, y, TRAIL_END(v->w)+1, y+TRAIL-1, 
                      v->light_shadow, v->mid_shadow, v->dark_shadow);
  if (v->show_ticks > 2)
    hdrawticks(v);
  ...
  else if (!v->active && v->himageinactive) 
  {
    ...
    x = (int)(vn*(TRAIL_LONG(v->w)-2) + TRAIL_START+1);
    cdPutImageRectMap(v->himageinactive_w, v->himageinactive_h, v->himageinactive, v->himageinactive_colors,
  ...
  else 
  {
    ...
    x = (int)(vn*(TRAIL_LONG(v->w)-2) + TRAIL_START+1);
    cdIupDrawRaisenRect(x-HANDLER_LONG/2-1, y, x+HANDLER_LONG/2, y+HANDLER_SHORT-1, 
                        v->light_shadow, v->mid_shadow, v->dark_shadow);

    /* handler background */
    cdForeground( v->bgcolor );
    cdBox(x-HANDLER_LONG/2, x+HANDLER_LONG/2-2, y+1, y+HANDLER_SHORT-1-2);

    /* handler sunken mark */
    cdIupDrawVertSunkenMark(x, y+3, y+HANDLER_SHORT-3, v->light_shadow, v->dark_shadow);
  }
}
...
static int motion_cb(Ihandle *self,int x,int y,char *r)
{
  ...
  if (!v->active) return IUP_DEFAULT;
...
{
  Tval *v=(Tval *)iupGetEnv(self,"_IUPVAL_DATA");
  cdCanvas *old_canvas=NULL;

  if (v->cdcanvas==NULL)
    v->cdcanvas=cdCreateCanvas(CD_IUP,self);

  /* guarda canvas ativo e ativa canvas CPI */
  old_canvas = cdActiveCanvas();
  cdActivate(v->cdcanvas);
  cdGetCanvasSize(&v->w,&v->h,NULL,NULL);
  if (v->im!=NULL)
  {
    cdKillCanvas(v->cdim);
    cdKillImage(v->im);
  }
  v->im=cdCreateImage(v->w,v->h);
  v->cdim=cdCreateCanvas(CD_IMAGE,v->im);

  /* restaura canvas ativo */
  if(old_canvas != NULL)
    cdActivate(old_canvas);
...
static void valuatordestroy(Ihandle *h)
{
  ...
  if (v==NULL) return;
  if (v->cdcanvas == NULL) return;

  if (v->cdim!=NULL) cdKillCanvas(v->cdim);
  if (v->im!=NULL) cdKillImage(v->im);
  if (v->cdcanvas!=NULL) cdKillCanvas(v->cdcanvas);
...
static void valuatorsetattr(Ihandle *h, char *attr, char *value)
{
  ...
  else if(iupStrEqual(attr,IUP_ACTIVE))
  {
    if(value==NULL) /* reseta para default */
      ...
      v->active = iupStrEqual(value,IUP_YES)? 1: 0;
  }
  ...
  if (IupGetAttribute(h, IUP_WID) != NULL && v->cdcanvas != NULL && v->cdim != NULL)
    drawval(v, h);
}
