 ...
 * $Id: imfocus.c,v 1.43 2006/03/15 02:51:55 scuri Exp $
 */
...
int iupmatFocusCb(Ihandle *hm, int focus)
{
  ...
  if (!iupStrEqual(IupGetType(hm), "matrix"))
    return IUP_IGNORE;

#ifdef _MOTIF_
  if(focus && IupGetInt(mat_canvash(mat), "_IUPMAT_DOUBLE_CLICK"))
  {
    IupSetAttribute(mat_canvash(mat), "_IUPMAT_DOUBLE_CLICK", NULL);
    return IUP_DEFAULT;
  }
#endif
