 ...
 * $Id: imgetset.c,v 1.28 2006/02/03 21:51:29 uid20021 Exp $
 */
...
void  iupmatSetOrigin(Ihandle *h, char *value)
{
 ...
 int visible = (iupCheck(h,IUP_VISIBLE)==YES) && (iupCheck(d,IUP_VISIBLE)==YES);
 int err;
 int lin,col,oldlin,oldcol,num=0;
 ...
 /* Pega os parametros. Um '*' indica que quero manter a tabela na mesma
   linha ou coluna.
 */
 if (sscanf(value,"%d:%d",&lin,&col) != 2)
 {
   if (sscanf(value,"%d:*",&lin) == 1)
     col = mat_fc(mat) + 1;
   else if(sscanf(value,"*:%d",&col) == 1)
     lin = mat_fl(mat) + 1;
   else
     return;
 }
