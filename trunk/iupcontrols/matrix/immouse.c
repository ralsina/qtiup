 ...
 * $Id: immouse.c,v 1.38 2006/03/15 02:51:55 scuri Exp $
 */
...
*/
static int CallClickCb(Ihandle *h, int lin, int col, char *r)
{
  Iitemrightclickcb cb;

  cb=(Iitemrightclickcb)IupGetCallback(h,IUP_CLICK_CB);
...
static void LeftPress(Ihandle *h, int x, int y, int shift, int ctrl, int duplo)
{
  ...
  if (iupmatColresTry(h,x,y))
    return;            /* Resize da largura de uma coluna iniciado */
...
*/
static void LeftRelease(Ihandle *h, int x, int y, char *r)
{
...
*/
static void RightRelease(Ihandle *h, int x, int y, char *r)
{
  int lin,col;
...
int iupmatMouseButtonCb (Ihandle *hm, int b, int press, int x, int y, char *r)
{
  ...
  else if (b==IUP_BUTTON3)
  {
    if (press)
      ...
      RightRelease(hm,x,y,r);
  }

  if (press)
  {
    iupmatGetLineCol(hm,x,y,&lin,&col);
    ret = CallClickCb(hm,lin,col,r);
    if (ret == IUP_IGNORE)
      return ret;
