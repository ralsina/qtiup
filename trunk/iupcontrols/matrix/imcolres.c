 ...
 * $Id: imcolres.c,v 1.23 2006/01/07 20:10:11 scuri Exp $
 */
...
void iupmatColresFinish(Ihandle *h, int x)
{
  ...
  if (Lastxpos != -1)
  {
    ...
    cdActivate(mat->cdh_buf); /* no double buffer here */

    y1 = mat_ht(mat);
    y2 = YmaxCanvas(mat);

    cdWriteMode(CD_XOR);
    cdForeground(RESIZE_COLOR);
    CdLine(Lastxpos,y1,Lastxpos,y2);
    cdWriteMode(CD_REPLACE);

    /* no flush here needed */
...
void iupmatColresMove(Ihandle *h, int x)
{
  ...
  /* no double buffer here */
  cdActivate(mat->cdh_buf);
...
void iupmatColresChangeCursor(Ihandle *h, int x, int y)
{
  ...
  if(mat_ht(mat) && y<mat_ht(mat) && ativo)
  {
    ...
    if (achou)
      iupCpiDefaultSetAttr(mat_canvash(mat),IUP_CURSOR,IUP_RESIZE_W);
    else
      setMatrixCursor(mat_canvash(mat)); /* Esta na area vazia apos a ultima coluna */
  }
  else
    setMatrixCursor(mat_canvash(mat));
}
...
void iupmatColresSet(Ihandle *h, int col, int m)
{
 ...
 if (m == MAT_COL)
 {
   if (col > mat_nc(mat) || col < 0) return;
   sprintf(aux, "WIDTH%d",col);
 }
 else
 {
   if (col > mat_nl(mat) || col < 0) return;
   sprintf(aux, "HEIGHT%d",col);
 }

 larg = iupGetEnvRaw(h,aux);
 sscanf(larg,"%d",&largura);
