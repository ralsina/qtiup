...
* $Id: imedit.c,v 1.10 2006/03/15 02:51:55 scuri Exp $
*/
...
static int iupmatDropCb (Ihandle *self, char *t, int i, int v)
{
  ...
  Dropmscb cb = (Dropmscb)IupGetCallback(mat_canvash(mat),IUP_DROPSELECT_CB);

  if (cb)
  {
    int ret = cb(mat_canvash(mat),mat_lin(mat)+1,mat_col(mat)+1,self,t,i,v);

    /* If the user returns IUP_CONTINUE in a dropselect_cb 
    the value is accepted and the matrix leaves edition mode. */
    if(ret == IUP_CONTINUE)
      iupmatEditClose(mat_canvash(mat));
  }
...
static int iupmatCallEditionCb(Ihandle *h, int modo, int update)
{
  ...
  if (cb)
  {
    ...
    /* the user callback may activate another CD canvas */
    if (cdActiveCanvas() != cdcv(mat))
      cdActivate(cdcv(mat));
  }
...
static int iupmatEditCancel(Ihandle *h, int focus, int update, int ignore)
{
  ...
  if (iupCheck(mat_edtdatah(mat), "VISIBLE")==YES)
  {
    ...
    if (IupGetInt(mat_canvash(mat), "_IUPMAT_CALL_EDITION")==1)
      return IUP_DEFAULT;

    iupSetEnv(mat_canvash(mat), "_IUPMAT_CALL_EDITION", "1");
    ret = iupmatCallEditionCb(h, 0, update);
    iupSetEnv(mat_canvash(mat), "_IUPMAT_CALL_EDITION", NULL);
...
#ifdef _MOTIF_
  Tmat *mat = (Tmat*)matrix_data(self);
  if(IupGetInt(mat_canvash(mat), "_IUPMAT_DOUBLE_CLICK"))
    return IUP_DEFAULT;
#endif
...
int iupmatEditShow(Ihandle *self)
{
  ...
  IupStoreAttribute(mat_edtdatah(mat),IUP_BGCOLOR,
                    iupmatDrawGetBgColor(mat_canvash(mat),mat_lin(mat)+1, mat_col(mat)+1));
  IupStoreAttribute(mat_edtdatah(mat),IUP_FGCOLOR,
                    iupmatDrawGetFgColor(mat_canvash(mat),mat_lin(mat)+1, mat_col(mat)+1));
...
{
  Tmat *mat = (Tmat*)matrix_data(h);
  ImatrixActioncb cb = (ImatrixActioncb) IupGetCallback(mat_canvash(mat),"ACTION_CB");

  /* Chama callback do usuario */
  if (cb)
  {
    int oldc = c;
    c = cb(mat_canvash(mat), c, mat_lin(mat)+1, mat_col(mat)+1, 1, after);
  ...
  case K_cLEFT:
  case K_cRIGHT:     
    if (iupmatEditClose(mat_canvash(mat)) == IUP_DEFAULT)
    {
      iupmatKey(mat_canvash(mat),c);  
      return IUP_IGNORE;
    }
    break;
  case K_LEFT:
  ...
  case K_CR:
    if (iupmatEditClose(mat_canvash(mat)) == IUP_DEFAULT)
      iupmatKey(mat_canvash(mat),c+1000);  
    break;
  }
...
static int iupmatKeyAnyDropCb(Ihandle *h, int c)
{
  ...
  {
    int oldc = c;
    c = cb(mat_canvash(mat), c, mat_lin(mat)+1, mat_col(mat)+1, 1, "");
    if (c==IUP_IGNORE || c==IUP_CLOSE  || c==IUP_CONTINUE)
      return c;
    else if (c==IUP_DEFAULT)
      c = oldc;
