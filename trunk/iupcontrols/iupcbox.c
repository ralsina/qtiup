 ...
 * $Id: iupcbox.c,v 1.10 2006/01/19 22:03:21 scuri Exp $
 */
...
static Ihandle* getCBox(Ihandle* self)
{
  ...
  while (parent && !iupStrEqual("cbox", IupGetType(parent)))
...
static int motion_cb(Ihandle *self,int x,int y,char *r)
{
  ...
  if (iupStrEqual("cbox", IupGetType(self)))
    return IUP_DEFAULT;

  IupSetAttribute(self, "CURCURSOR", "HAND");
  ...
  iupdrvUpdate(IupGetDialog(self));
