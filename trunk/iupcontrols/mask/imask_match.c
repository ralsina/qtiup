 ...
 * $Id: imask_match.c,v 1.1 2006/02/04 21:10:17 uid20021 Exp $
 */
...
static match_t match_blanks (char *text, long j)
{
  return (((text[j] == '\t') || (text[j] == '\xff') || (text[j] == ' ') ||
     (text[j] == '\n'))) ? NORMAL_MATCH : NO_MATCH;
}

static match_t match_non_blanks (char *text, long j)
{
...
static match_t match_alpha (char *text, long j)
{
  return (isalpha (text[j])) ? NORMAL_MATCH : NO_MATCH;
}

static match_t match_non_alpha (char *text, long j)
{
  return (!isalpha (text[j]) && (text[j] != '\0'))
    ? NORMAL_MATCH : NO_MATCH;
}

static match_t match_digit (char *text, long j)
{
  return (isdigit (text[j])) ? NORMAL_MATCH : NO_MATCH;
}

static match_t match_non_digit (char *text, long j)
{
  return (!isdigit (text[j]) && (text[j] != '\0'))
    ? NORMAL_MATCH : NO_MATCH;
}

static match_t match_alphanum (char *text, long j)
{
  return isalphanum (text[j]) ? NORMAL_MATCH : NO_MATCH;
}

static match_t match_non_alphanum (char *text, long j)
{
  return (isalphanum (text[j]) || (text[j] == '\0'))
    ? NO_MATCH : NORMAL_MATCH;
}

static match_t match_word_boundary (char *text, long j)
{
...
************************/
static long local_match (char *text, iMask * fsm, long start, char *addchar, int casei)
{
...
/******************************************************/
int iMaskMatch (char *text, iMask * fsm, long start, iMaskMatchFunc function, void *user, char *addchar, int icase)
{
  long ret;
