 ...
 * $Id: itgetset.c,v 1.45 2006/03/24 01:39:47 scuri Exp $
 */
...
int treegetsetSetValue(Ihandle *h, char* value, int call_cb)
{
  ...
  if(iupStrEqual(value,IUP_ROOT))
  {
    ...
    IupSetAttribute(tree_canvash(tree),IUP_POSY,"0.0");
  ...
  else if(iupStrEqual(value,IUP_LAST))
  {
    ...
    sprintf(buffer,"%f",1.0-IupGetFloat(tree_canvash(tree),IUP_DY));
    IupStoreAttribute(tree_canvash(tree),IUP_POSY,buffer);
 ...
 */
int treegetsetSetImage(char* name, char* image, unsigned long int* color, unsigned long int* marked_color, int range, Node node)
{
