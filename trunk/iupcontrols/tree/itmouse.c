 ...
 * $Id: itmouse.c,v 1.35 2006/03/24 01:39:47 scuri Exp $
 */
  ...
  if(node)
  {
    ...
    else
    {
      if(willmark || node_kind(node) != BRANCH)
      {
        ...
        if (tree_shift != YES || !IupGetCallback(h,"MULTISELECTION_CB"))
        {
...
{
  return treeInsideRegion(-treedrawGetLeft(h)+xmouse,
...
int treeText(Ihandle* h, Node node, int text_x, int xmouse, int ymouse, int x, int y)
{
  return (node_name(node) &&
      treeInsideRegion(-treedrawGetLeft(h)+xmouse,
    	                  treedrawGetCanvasTop(h)+ymouse,
			  x + NODE_WIDTH,
			  y - TEXT_BOX_OFFSET_Y,
