...
void sism4 ( tpoint *pt, char *data_rec )
{
   ...
   if ( n>0 )
    {
     if ( cordep )
      {
       cor = cdEncodeColor ( (unsigned char)((color_table[coind[i]].red*255)/cgm.color_ext.white.red),
                             (unsigned char)((color_table[coind[i]].green*255)/cgm.color_ext.white.green),
                             (unsigned char)((color_table[coind[i]].blue*255)/cgm.color_ext.white.blue) );
       cdForeground ( cor );
      }
     ...
     for ( j=0; j<n; j++ )
      cdVertex ( vdcx2canvas(x[j]), vdcy2canvas(y[j]) );
 ...
 for ( i=0; i<n_samp; i++ )
  {
   ...
   cor = cdEncodeColor ( (unsigned char)((color_table[index].red*255)/cgm.color_ext.white.red),
                         (unsigned char)((color_table[index].green*255)/cgm.color_ext.white.green),
                         (unsigned char)((color_table[index].blue*255)/cgm.color_ext.white.blue) );
   cdForeground ( cor );

   cdBegin ( CD_FILL );

   cdVertex ( vdcx2canvas(posdx + trace_start_pos.x + factx*(trace-1)),
              vdcy2canvas(trace_start_pos.y + facty*(i)) );
   cdVertex ( vdcx2canvas(posdx + trace_start_pos.x + factx*(trace-1)),
              vdcy2canvas(trace_start_pos.y + facty*(i+1)) );
   cdVertex ( vdcx2canvas(negdx + trace_start_pos.x + factx*(trace-1)),
              vdcy2canvas(trace_start_pos.y + facty*(i+1)) );
   cdVertex ( vdcx2canvas(negdx + trace_start_pos.x + factx*(trace-1)),
              vdcy2canvas(trace_start_pos.y + facty*(i)) );
 ...
 for ( i=0; i<n_samp; i++ )
  {
   ...
   if ( sample[i]>negclp && sample[i]<posclp && 
    ...
    cdVertex ( vdcx2canvas(sample[i] + dx), vdcy2canvas(y1) );
   else if ( sample[i]<negclp && sample[i+1]>negclp && sample[i+1]<posclp )
    cdVertex ( vdcx2canvas(negclp + dx), vdcy2canvas(interpl(i,y1,y2,negclp)) );
   else if ( sample[i]<negclp && sample[i+1]>posclp )
    {
     cdVertex ( vdcx2canvas(negclp + dx), vdcy2canvas(interpl(i,y1,y2,negclp)) );
     cdVertex ( vdcx2canvas(posclp + dx), vdcy2canvas(interpl(i,y1,y2,posclp)) );
    }
